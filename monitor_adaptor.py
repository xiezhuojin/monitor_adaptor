...
class MonitorAdaptor:
    ...
    def update_track(self, tracks: List[Track], clear_timeout=5) -> None:
        ...
        for id in self.tracks.keys():
            self.tracks[id] = [track for track in self.tracks[id]
                               if now - track.track_at <= clear_timeout]
        ...
        message = f"""
            ...
            app.$refs.map.trackLines.showTracks(parameter, ids);
        """
        self.mq.put(message)
    ...
    def add_device(self, device: Device) -> None:
        message = f"""
            ...
            app.$refs.map.devices.addDevice(parameter);
        """
        self.mq.put(message)
    ...
    def add_zone(self, zone: Zone) -> None:
        ...
        message = f"""
            ...
            app.$refs.map.zones.addZone(parameter);
        """
        self.mq.put(message)
...
if __name__ == "__main__":
    ...
    sleep(0.5)
    monitor_adaptor.add_device(Device("1", "horn", 113.306646, 23.383048, "horn1", True))
    sleep(2)
    ...
    sleep(0.5)
    monitor_adaptor.add_zone(Zone("1", "danger",
    ...
    while True:
        tracks = [get_random_track() for i in range(3000)]
